...
def communicate_with_server(endpoint, payload, max_retries=3):
  ...
  Returns:
    tuple: (success: bool, rd: dict or None)
  """
  ...
  for attempt in range(max_retries):
    try:
      ...
      if response.sc == 200:
        ...
        print(f"[ERROR] Server returned status code {response.sc}")
        try:
...
def activate_license(lk, hw):
  """
  Activate a license key with the server.
  
  Args:
    lk: The license key to activate
    hw: The hardware ID of this machine
...
def check_license_status(lk, hw):
  """
  Check license status and get a new JWT token.
  
  Args:
    lk: The license key to check
    hw: The hardware ID of this machine
...
def main():
  ...
  if len(sys.argv) < 2:
    print("\nUsage:")
    print("  python3 aegis_license_client.py <lk> [--test]")
    print("  python3 aegis_license_client.py --check")
    print("\nOptions:")
    print("  <lk>  : Your Aegis OS license key")
  ...
  if sys.argv[1] == '--check':
    ...
    success, token = check_license_status(lk, hw)
  else:
    lk = sys.argv[1]
    use_test_hwid = '--test' in sys.argv
    
    hw = generate_hardware_id(use_test_hwid)
    print(f"\n[INFO] Hardware ID: {hw}")
    
    success, token = activate_license(lk, hw)
